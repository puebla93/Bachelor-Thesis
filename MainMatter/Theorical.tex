...
Muchos algoritmos de \textit{BS} han sido diseñados para segmentar los objetos de primer plano del fondo de una secuencia (ver Fig. \ref{fig:BSExample}), y generalmente comparten el mismo esquema (ver Fig. \ref{fig:BSParadigm}).
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=10cm]{Graphics/LinearRegression.png}
...
\begin{algorithm}[!h]
    \DontPrintSemicolon
	\SetAlgoLined
	\KwData{data - conjunto de puntos de datos observados\\
            \hspace{1cm}model - modelo que pueda ser ajustado a los datos\\
            \hspace{1cm}n - número mínimo de puntos necesarios para ajustar el\\
            \hspace{1.5cm}modelo\\
            \hspace{1cm}k - cantidad máxima de iteraciones permitidas en el algoritmo\\
            \hspace{1cm}t - valor de umbral para determinar cuándo un punto se ajusta\\
            \hspace{1.5cm}al modelo\\
            \hspace{1cm}d - número de puntos cercanos necesarios para afirmar que un\\
            \hspace{1.5cm}modelo se ajusta bien a los datos}
    \KwResult{bestfit - parámetros del modelo que mejor se ajustan a los\\
            \hspace{2.7cm}datos (o \textit{nil} si no se encuentra un modelo bueno)}
    \everypar={\nl}
    iterations = 0\;
    bestfit = nil\;
    besterr = número realmente grande\;
    \While{$iterations < k$}{
	maybeinliers = \textit{n} valores aleatorios de los datos\;
	maybemodel = parámetros del modelo ajustados a \textit{maybeinliers}\;
    alsoinliers = conjunto vacio\;
	\For {cada punto en data que no está en maybeinliers} {
		\If{punto se ajusta a maybemodel con un error menor que t}
		    {agregar punto a \textit{alsoinliers}\;}
	}
	\If{la cantidad de elementos en alsoinliers es > d} {
        /* esto implica que podemos haber encontrado un buen\\
        modelo, ahora prueba qué tan bueno es */\\
        bettermodel = parámetros del modelo ajustados a todos los\\
        \hspace{2.5cm}puntos en \textit{maybeinliers} y \textit{alsoinliers}\;
        thiserr = medida de qué tan bien el modelo se ajusta a estos\\
        \hspace{1.5cm}puntos\;
		\If {thiserr < besterr} {
			bestfit = bettermodel\;
			besterr = thiserr\;
...
Como se describe al principio del algoritmo, \textit{RANSAC} necesita algunos parámetros predefinidos para ejecutarse: tamaño del subconjunto de muestra ($n$), umbral de tolerancia de error ($t$), umbral de consenso mínimo ($d$) y número de iteraciones ($k$).
